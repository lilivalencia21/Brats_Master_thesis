...
exp_dice_cfg = {'patch_shape' : (32, 32, 32),
               'step' : (12, 12, 12),
               'epochs': 10,
               'model_name': 'DiceLoss',
                'patience': 3 }

exp_dice_cfg.update({'sampler' : UniformSampler(experiment_cfg['patch_shape'], experiment_cfg['step'], num_elements=None)})


""" NoNew Net configuration to test"""

params_try1 = {'batch_size':128,
          'shuffle': True,
          'num_workers': 64}

experiment_try1_cfg = {'patch_shape' : (32, 32, 32),
               'step' :  (12, 12, 12),
               'sampler' : UniformSampler,
               'epochs': 10,
                'model': UNet3D(),
               'model_name': 'DiceLossTry1',
                'patience': 3,
                'pathToSaveTxt':'/home/liliana/dataToValidate/DiceLossUned3D'}

experiment_try1_cfg.update({'sampler' : UniformSampler(experiment_try1_cfg['patch_shape'], experiment_try1_cfg['step'],
                                                       num_elements=None)})


cross_validation(dataset, params_try1, experiment_try1_cfg)
