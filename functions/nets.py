...
class UNet3DNNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.out = nn.Conv3d(30, 4, kernel_size=1)
    ...
    def forward(self, x):
        with torch.autograd.set_detect_anomaly(True):
            ...
            # skipCon1 = torch.add(block4, block6)
            block7 = F.interpolate(F.leaky_relu(self.up2(block6)), scale_factor=2, mode='trilinear')
            # skipCon2 = torch.cat((block3, block7))
            block8 = F.interpolate(F.leaky_relu(self.up3(block7)), scale_factor=2, mode='trilinear')
            # skipCon3 = torch.cat((block2, block8))
            block9 = F.interpolate(F.leaky_relu(self.up4(block8)), scale_factor=2, mode='trilinear')
            # skipCon4 = torch.cat((block1, block9))
